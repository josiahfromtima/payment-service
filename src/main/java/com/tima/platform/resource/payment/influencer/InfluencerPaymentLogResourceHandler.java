...
public class InfluencerPaymentLogResourceHandler {
    ...
    public Mono<ServerResponse> getAllTransactions(ServerRequest request)  {
        ...
        log.info("Get All Transactions For Influencer Requested", request.remoteAddress().orElse(null));
        return jwtAuthToken
    ...
    public Mono<ServerResponse> getTransactionsByStatus(ServerRequest request) {
        ...
        log.info("Get Transactions By Status Requested", request.remoteAddress().orElse(null));
        return jwtAuthToken
                .map(ApiResponse::getPublicIdFromToken)
    ...
    public Mono<ServerResponse> getTransactionsByBrand(ServerRequest request) {
        ...
        log.info("Get Transactions By Brand Name Requested ", brand, request.remoteAddress().orElse(null));
        return jwtAuthToken
                .map(ApiResponse::getPublicIdFromToken)
